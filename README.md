...
Each sweep:
1. Finds all `checkpoints/d1_N/` directories
2. Runs each on the 100 held-out eval problems (greedy decoding, temperature=0)
3. Prints a results table
4. Saves a PNG plot (avg_questions on x-axis, pass@1 on y-axis, each point labelled with its d₁)
...
2. **False leads** - `top_k=0`, reducing PPO epochs, disabling LoRA dropout - none addressed the remaining ~3
3. **Root cause** - the thorough debug script revealed the logit processor filtering, leading to the forward pass fix that resolved the issue completely
