...
class UserSimulator:
    ...
    def __init__(self, cfg):
        ...
        self._semaphore = asyncio.Semaphore(cfg.user_simulator.max_concurrent_api)
    ...
    async def _answer_count(self, question: str, original_prompt: str) -> Tuple[str, int]:
        ...
        answer_text, question_count = _parse_count_response(raw)
        return answer_text, question_count

    async def _answer_truncate(self, question: str, original_prompt: str) -> Tuple[str, int]:
        # Only pass the first question to the simulator
...
def _parse_count_response(raw: str) -> Tuple[str, int]:
    ...
    """
    pattern = re.compile(r"\nQUESTION_COUNT:\s*(\d+)\s*$", re.IGNORECASE)
