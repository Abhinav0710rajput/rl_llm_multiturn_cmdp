...
class PPOLagrangianTrainer:
    ...
    def _update_step(
        ...
        for t in transitions:
            new_logp, hidden, ref_logp = self.agent.score(t.prompt, t.action_ids)
    ...
    def save_checkpoint(self, iteration: int, final: bool = False):
        ...
        # LoRA weights + tokenizer
        self.agent.save_lora(ckpt_dir)

        # Value heads
        torch.save(self.value_heads.state_dict(),
                   os.path.join(ckpt_dir, "value_heads.pt"))

        # Dual variables
        torch.save(self.dual.state_dict(),
                   os.path.join(ckpt_dir, "dual_variables.pt"))
